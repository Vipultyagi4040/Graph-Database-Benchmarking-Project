...
export class ArangoAdapter extends BaseAdapter {
  ...
  async traversal(startId, hops) {
    const cursor = await this.db.query(
      `FOR v IN ${hops}..${hops} OUTBOUND @start GRAPH @graph RETURN DISTINCT v`,
      { start: `${NODE_COLLECTION}/${startId}`, graph: GRAPH_NAME }
    );
  ...
  async pointLookup(id) {
    const cursor = await this.db.query(
      `FOR d IN ${NODE_COLLECTION} FILTER d.nid == @id RETURN d`,
      { id }
    );
  ...
  async indexedLookup(age) {
    const cursor = await this.db.query(
      `FOR d IN ${NODE_COLLECTION} FILTER d.age == @age COLLECT WITH COUNT INTO c RETURN c`,
      { age }
    );
  ...
  async aggregation() {
    const cursor = await this.db.query(
      `FOR d IN ${NODE_COLLECTION} COLLECT age = d.age WITH COUNT INTO c SORT age RETURN {age, c}`
    );
    return (await cursor.all()).length;
  }
  ...
  async write(id) {
    await this.db.query(
      `FOR d IN ${NODE_COLLECTION} FILTER d.nid == @id UPDATE d WITH {touched: DATE_NOW()} IN ${NODE_COLLECTION}`,
      { id }
    );
  }
